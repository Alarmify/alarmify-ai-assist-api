...
import (
	...
	"ai-assist-api/internal/handlers"
	"log"
	"net/http"
	"time"
...
func RegisterAPIRoutes(r *Router, handler *handlers.APIHandler) {
	v1 := r.Group("/api/v1")
	{
		...
		{
			incidents.POST("/analyze", handler.AnalyzeIncident)
		}
		// /runbooks routes
		runbooks := v1.Group("/runbooks")
		{
			runbooks.POST("/suggest", handler.SuggestRunbook)
		}
		// /alerts routes
		alerts := v1.Group("/alerts")
		{
			alerts.POST("/correlate", handler.CorrelateAlerts)
		}
		// /query routes
		query := v1.Group("/query")
		{
			query.POST("/", handler.NaturalLanguageQuery)
		}
	}
}
